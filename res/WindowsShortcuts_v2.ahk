...
Class WindowsShortcuts { 
    ...
    ; New : 25-01-24
    ; ShowWindowsShortcuts : () : Display Windows Shortcuts reference GUI with search functionality
    ShowWindowsShortcuts() {
        ...
        categoriesAndShortcuts := this.CreateShortcutsData()
        ...
        SearchFunc() {
            ...
            moduleCore.logToFile('Searching for: ' searchTerm)

            if (searchTerm = '') {
                ; If search is empty, restore original content
                mainEdit.Value := originalContent
                moduleCore.logToFile('Search cleared - restored original content')
            } else {
                ; Build filtered content based on search term
                filteredContent := this.BuildFilteredContent(categoriesAndShortcuts, searchTerm)
                mainEdit.Value := filteredContent
                moduleCore.logToFile('Search completed for: ' searchTerm)
            }
        }
        ...
        shortcutGui.Show('w1500 h1500')

        moduleCore.logToFile('Windows Shortcuts GUI displayed with single scrollable list')
    }

    ; New : 25-01-24
    ; BuildAllCategoriesText : (categoriesAndShortcuts) : Build complete text content with all categories
    ; categoriesAndShortcuts : Map - Map of categories to shortcut arrays
    ; Returns : string - Formatted text with all shortcuts
    BuildAllCategoriesText(categoriesAndShortcuts) {
        ...
        for category, shortcuts in categoriesAndShortcuts {
            ...
            allContent .= this.BuildCategoryText(shortcuts, false) ; Don't add extra spacing
    ...
    ; New : 25-01-24
    ; BuildFilteredContent : (categoriesAndShortcuts, searchTerm) : Build filtered content based on search term
    ; categoriesAndShortcuts : Map - Map of categories to shortcut arrays  
    ; searchTerm : string - Search term to filter by
    ; Returns : string - Formatted filtered results or 'no results' message
    BuildFilteredContent(categoriesAndShortcuts, searchTerm) {
        ...
        for category, shortcuts in categoriesAndShortcuts {
            ...
            if (categoryResults.Length > 0) {
                ...
                filteredContent .= this.BuildCategoryText(categoryResults, false)
                filteredContent .= '`r`n`r`n'
            }
        }
    ...
    ; New : 25-01-24
    ; BuildCategoryText : (shortcuts, addSpacing) : Build formatted text content for a category
    ; shortcuts : array - Array of shortcut objects with key and desc properties
    ; addSpacing : bool - Whether to add extra spacing (optional)
    ...
    ; New : 25-01-24
    ; CreateShortcutsData : () : Create shortcut data organized by category
    ; Returns : Map - Map of categories to arrays of shortcut objects
    CreateShortcutsData() {
        ; NMS 1 line added

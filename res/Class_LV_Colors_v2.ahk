...
Class LV_Colors {
   ...
   ; Clears all row and cell colors
   ...
   ; Updates the RowCount, ColCount, BkClr, and TxClr properties
   UpdateProps() {
      If !(This.HWND)
         Return False
   ...
   ; Sets background and/or text color for even row numbers
   ...
   ; Sets background and/or text color for even column numbers
   ...
   ; Sets background and/or text color for selected rows
   ...
   ; Sets background and/or text color for the specified row
   ...
   ; Sets background and/or text color for the specified cell
   ...
   ; Prevents/allows sorting by click on a header item for this ListView
   NoSort(Apply := True) {
      If !(This.HWND)
         Return False
   ...
   ; Prevents/allows resizing of columns for this ListView
   NoSizing(Apply := True) {
      If !(This.Header)
         Return False
   ...
   ; Adds/removes a message handler for NM_CUSTOMDRAW notifications of this ListView
   ...
   ; Internal message handler for NM_CUSTOMDRAW notifications
   NM_CUSTOMDRAW(LV, L) {
   ...
   ; Provides the unique internal ID of the given row number
   MapIndexToID(Row) {
      Return SendMessage(0x10B4, Row - 1, 0, This.HWND) ; LVM_MAPINDEXTOID
   }
   ; Converts colors to BGR format
   BGR(Color, Default := "") {
